...
(function() {
  ...
  async function loadCompliance(deviceId) {
    ...
    try {
      ...
      // Overall score
      setText('.compliance-ring-num', (data.overallScore || data.securityScore || '--') + '%');
      setText('.compliance-ring-grade', 'Grade ' + (data.overallGrade || data.securityGrade || '--'));

      // Ring SVG fill (circumference ~251 for r=40)
      const ring = document.querySelector('.compliance-ring circle:last-child');
      if (ring) {
        const offset = 251 - (251 * (data.overallScore || data.securityScore || 0) / 100);
        ring.style.strokeDashoffset = offset;
      }

      // Summary
      const summaryH3 = document.querySelector('.compliance-summary h3');
      const fws = data.frameworks || data.categories || [];
      const _total = data.totalChecks || fws.reduce((s,f) => s + (f.totalControls || 0), 0);
      const _passed = data.passedChecks || fws.reduce((s,f) => s + (f.passedControls || 0), 0);
      const _failed = data.failedChecks || fws.reduce((s,f) => s + (f.failedControls || 0), 0);
      if (summaryH3) summaryH3.textContent = _passed + '/' + _total + ' Controls Passed';
      const summaryP = document.querySelector('.compliance-summary p');
      if (summaryP) summaryP.textContent = _failed + ' gaps identified across ' + fws.length + ' categories';

      // Framework cards - map categories to framework cards
      const fwCards = document.querySelectorAll('.framework-card');
      if (fwCards.length > 0 && (data.frameworks || data.categories)) { const _fws = data.frameworks || data.categories;
        
        fwCards.forEach((card, i) => {
          const cat = _fws[i];
          if (!cat) return;
          const nameEl = card.querySelector('.framework-name');
          if (nameEl) nameEl.textContent = cat.frameworkName || cat.category || cat.name;
          const valEl = card.querySelector('.mini-ring-value');
          if (valEl) valEl.textContent = Math.round(cat.scorePercent || cat.compliancePercent || 0) + '%';
          const passedEl = card.querySelector('.framework-stats .passed');
          if (passedEl) passedEl.textContent = (cat.passedControls || cat.passedChecks || 0) + ' passed';
          const gapsEl = card.querySelector('.framework-stats .gaps');
          if (gapsEl) gapsEl.textContent = ((cat.totalControls || cat.totalChecks || 0) - (cat.passedControls || cat.passedChecks || 0)) + ' gaps';
          // Mini ring
          ...
          if (miniRing) {
            ...
            miniRing.style.strokeDashoffset = c - (c * (cat.scorePercent || cat.compliancePercent || 0) / 100);
          }
        });
      }
      ...
      if (gapSections.length > 0 && (data.frameworks || data.categories)) {
        const allFailed = [];
        (data.frameworks || data.categories).forEach(cat => {
  ...
  async function loadScanResults(deviceId) {
    ...
    try {
      ...
      if (nums.length >= 4) {
        nums[0].textContent = _total;
        nums[1].textContent = _passed;
        const warnings = _total - _passed - _failed;
        nums[2].textContent = warnings > 0 ? warnings : 0;
        nums[3].textContent = _failed;
      }
      setText('.summary-last-scan', 'Last scan: ' + fmtDate(data.lastScanned || new Date().toISOString()));

      // Category sections
      const sections = document.querySelectorAll('.category-section');
      if (sections.length > 0 && (data.frameworks || data.categories)) {
